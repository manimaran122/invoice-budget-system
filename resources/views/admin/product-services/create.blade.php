    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-app-card border border-app-border overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.product-services.store') }}" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="price" name="price" type="number" step="0.01" min="0" class="mt-1 block w-full" value="{{ old('price', 0) }}" required />
                            <x-input-error :messages="$errors->get('price')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="tax_percentage" value="Tax %" />
                            <x-text-input id="tax_percentage" name="tax_percentage" type="number" step="0.01" min="0" max="100" class="mt-1 block w-full" value="{{ old('tax_percentage', 0) }}" required />
