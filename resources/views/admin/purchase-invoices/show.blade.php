<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.purchase-invoices.index') }}" class="no-print text-sm font-medium text-app-muted hover:text-app-dark">
                Back
            </a>
        </div>
    </x-slot>
            ...
            <div id="invoice-print-area" class="bg-app-card border border-app-border shadow-sm sm:rounded-lg">
                <div class="border-b border-app-border p-6 sm:p-8">
                    <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-widest text-primary">Purchase Invoice</p>
                            <h1 class="mt-2 text-3xl font-bold text-app-dark">#{{ $purchaseInvoice->invoice_number }}</h1>
                            <p class="mt-2 text-sm text-app-muted">Invoice Date: {{ $purchaseInvoice->invoice_date->format('Y-m-d') }}</p>
                            <p class="text-sm text-app-muted">Due Date: {{ $purchaseInvoice->due_date?->format('Y-m-d') ?? '-' }}</p>
                        </div>
                ...
                <div class="grid gap-6 border-b border-app-border p-6 sm:grid-cols-2 sm:p-8">
                    <div>
                        ...
                        <div class="mt-3 text-sm text-app-dark">
                            <p class="font-semibold">{{ $purchaseInvoice->supplier?->name ?? '-' }}</p>
                            <p>{{ $purchaseInvoice->supplier?->email ?? '-' }}</p>
                            <p>{{ $purchaseInvoice->supplier?->phone ?? '-' }}</p>
                            <p class="text-app-muted">{{ $purchaseInvoice->supplier?->address ?? '-' }}</p>
                        </div>
                    </div>
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="mt-8 flex justify-end">
                        <div class="w-full max-w-sm space-y-3">
                            ...
                            <div class="flex justify-between text-sm text-app-muted">
                                <span>Discount</span>
                                <span>{{ number_format($purchaseInvoice->discount, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-sm text-app-muted">
                                <span>Tax</span>
                                <span>{{ number_format($purchaseInvoice->tax, 2) }}</span>
                            </div>
                            <div class="flex justify-between border-t border-app-border pt-3 text-lg font-bold text-app-dark">
                                <span>Grand Total</span>
                                <span>{{ number_format($purchaseInvoice->total, 2) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="no-print mt-8 flex justify-end gap-3">
                        <a href="{{ route('admin.purchase-invoices.index') }}" class="rounded-md border border-app-border px-4 py-2 text-sm font-semibold text-app-muted hover:bg-app-background">
                            Back
                        </a>
