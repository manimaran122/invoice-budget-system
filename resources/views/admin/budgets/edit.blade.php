    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-app-card border border-app-border overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.budgets.update', $budget) }}" class="p-6 space-y-4">
                    @csrf
                    @method('PATCH')

                    <div>
                        <x-input-label for="name" value="Budget Name" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name', $budget->name) }}" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="mt-1 block w-full rounded-md border-app-border shadow-sm focus:border-primary focus:ring-primary" required>
                            ...
                            <option value="Monthly" @selected(old('type', $budget->type) === 'Monthly')>Monthly</option>
                            <option value="Yearly" @selected(old('type', $budget->type) === 'Yearly')>Yearly</option>
                        </select>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="amount" name="amount" type="number" step="0.01" min="0" class="budget-amount mt-1 block w-full" value="{{ old('amount', $budget->amount) }}" required />
                            <x-input-error :messages="$errors->get('amount')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="spent" value="Spent" />
                            <x-text-input id="spent" name="spent" type="number" step="0.01" min="0" class="budget-amount mt-1 block w-full" value="{{ old('spent', $budget->spent) }}" />
